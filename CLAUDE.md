...
| Date (WIB)       | Change                                                                                               |
| ---------------- | ---------------------------------------------------------------------------------------------------- |
| 2026-09-18 16:07 | **Declining the break at a task checkpoint resets the Pomodoro clock (issue #39).** *Complete*, *Pending* and *Extend* at a work checkpoint set `pomodoro_since = now` (when the mode is on), as *Skip & continue* already did — otherwise a checkpoint 20 min into a pomodoro re-prompted 5 min later. `decline_break` in `core/timer_machine.rs`. `docs/features/POMODORO_MODE/SPEC.md` D53, §3.2, test 110. |
| 2026-09-18 15:12 | **The work checkpoint shows *Up next* (issue #38).** `Snapshot.nextTask` is the task *Start Next* would start, other than the current one, or `null`. It is `MachineState::first_startable(day_start, skip)`, which `start_next` now calls too (skip `None`) — one query, so the name shown and the task started cannot disagree. Presentation plus one snapshot field; no reducer behaviour changed. `docs/SPEC.md` §7.4, test 109. |
| 2026-09-16 13:41 | **13.3 Energy Impact while a block ran (issue #36).** Three costs, all in the 1 Hz path and none of them a rule. (1) `App::dispatch` persisted unconditionally, so every tick rewrote every task, block and span in a transaction — but `Event::Tick` mutates the state only on expiry or a Pomodoro falling due, so ~3600 WAL writes an hour stored what was already there. It now **compares the reduced state with the one it held** and saves only on a difference; the test is equality, not the event, so a rejected `SwitchTo` at a checkpoint writes nothing either. The only column a no-op save moved was `app_state.updated_at`, which nothing reads. (2) `timebox://changed` was **broadcast**, and windows are hidden rather than destroyed — so the popover, the main window and the checkpoint each paid a full `get_snapshot` (whole `MachineState` serialised, summary recomputed over every span) and a whole-tree re-render once a second while invisible. `emit_to_visible` in `lib.rs` filters the tick emit by `is_visible()`; the other emits stay broadcast, and a hidden window is caught up by the `Focused(true)` emit and the 10s stopped-state poll. (3) `Countdown`'s local interpolation ran at **250ms** for a `mm:ss` display, in every open window. Now 1000ms, with the backend's own nudge landing between. `docs/SPEC.md` §4.5, §6, test 108. |
| 2026-09-14 15:26 | **Popover width is 380px**, not the 340 below — widened again after seeing it run. Both `WIDTH` constants changed together. |
| 2026-09-14 15:22 | **Popover widened 300 → 340px** so task titles truncate less. The width is `WIDTH` in **two** places that must agree: `platform/popover.rs` builds and centres the window under the tray icon with it, and `components/Popover.tsx` passes it to `setSize` each time the card re-measures its height — change one alone and the window is resized to the other's width on first paint, off-centre. `docs/SPEC.md` §7.2. |
| 2026-09-14 15:13 | **The `daily` chip on the popover's list and the overlay (issue #33).** Extracted from `Queue.tsx` to `DailyBadge` in `components/ui.tsx`, beside `PriorityDot`, and used by all three. The overlay puts it on the title line, not a new row — the card's height is fixed by Rust (D51). Presentation only. `docs/features/DAILY_TASKS.md` §5, `TIMER_OVERLAY.md` §3. |
| 2026-09-14 14:03 | **The popover lists dailies done today last (issue #31).** Ticked dailies near the head filled its five-row preview. `core::queue::done_last` builds `Snapshot.popoverQueue` (`state.queue` with `doneToday` moved below, each group in stored order) and only the popover reads it. **Display order, not a reorder**: moving the daily in `state.queue` would leave it at the bottom tomorrow, and `AddTask` appends, so a task added after the tick would land beneath it and be cut off anyway. The main window keeps `state.queue`, where order is dragged. `docs/features/DAILY_TASKS.md` §5.2, test 107. |
| 2026-09-10 11:35 | **A running break read as idle on the overlay (issue #27).** `components/Overlay.tsx` tested `!task` for *nothing running*, and a break block carries no task by design — so the card labelled itself *Break* and then showed today's idle total where the break countdown belongs. The test is now `Idle`, or no task **and** no break. Presentation only. |
| 2026-09-10 11:27 | **The overlay never updated (issue #27)** — it painted the block it mounted with and nothing after: the same 00:00 through the next block, and *Focus 29:15* while a break ran. `src-tauri/capabilities/default.json` lists the windows a capability applies to, and `overlay` was not among them (nor `settings`). A window left out still runs the app's own `#[tauri::command]`s — those need no permission — so `get_snapshot` worked and the card looked right once; but `listen` is a **core** command, so it was refused and no `timebox://changed` was ever delivered. Nothing is visible from Rust: the emit succeeds and the window is there. The card even looked alive, because `Countdown` interpolates between nudges — it counted its first block down and froze. `useTimebox.init` also awaited `listen` bare, so the rejection took the 10s stopped-state poll down with it and removed the one thing that would have healed it; it is now caught, recorded as an error and the poll installed either way. **Adding a window means adding its label to that list.** `docs/features/TIMER_OVERLAY.md` §3.1. |
| 2026-09-10 09:55 | **Timer overlay (issue #23).** A floating always-on-top card on the main display showing the current block — countdown and task title, a break, or today's idle total when nothing is running. Four settings columns (migration **007**) and no domain change at all: `platform/overlay.rs` owns the window and `reconcile(app, settings, state)` is called from startup, every `dispatch`, `update_settings` and the tick loop, so the window is purely a consequence of stored state (D52) rather than a second source of truth an `Action` would create. It is a **display, not a control** — `set_ignore_cursor_events` means it can never take focus or swallow a click, which is also why the position is five named anchors and not a drag (D48, D51). It **hides at either checkpoint** (D49): the checkpoint fills the same display and must carry the screen alone. Opacity is the **window's alpha** (`NSWindow.setAlphaValue:`), not the card's fill — fading only the background needs the private API the App Store rules out, so the text fades with it (D50). Size is fixed at 200×96 (or ×68 with the title off) because the card is corner-anchored and a self-resizing webview would wobble against Rust re-placing it. Geometry reuses `checkpoint::main_monitor_frame` — logical points, the monitor's own scale factor (issue #20). `docs/features/TIMER_OVERLAY.md`, D47–D52, tests 102–106. |
| 2026-09-10 09:35 | **The current block now sits on a tinted panel (issue #22)** — the popover and the main window. State was legible only by reading — the title, the countdown colour, which buttons were present. `components/Popover.tsx` paints the current section `accent-soft` while a work block runs, `surface-2` while it is paused and `rest-soft` during a break; at **either** checkpoint it stays plain, because the waiting banner inside it is already `alert-soft`/`rest-soft` and a tint behind it would erase the one thing that must be seen. The section was one flat block, so the tint would have swallowed the *Take a break* and *Pomodoro mode* rows as well — those are controls, not state — and it is now split: a padded, tinted block for the current task, and an unpadded sibling holding the two divider rows (their `mt-2.5 … pt-2.5` becomes `py-2.5`, same spacing), with *Take a break* taking a band of its own that fades from `surface` into `alert-soft` — chosen by eye against the reference and knowingly against the palette's grammar, since `alert-soft` is otherwise the checkpoint's colour; the row is hidden at either checkpoint (`canBreak`), so the two never appear together. The fade starts at `surface` rather than `transparent`, which WebKit interpolates through transparent *black* and greys the top of the gradient. `components/CurrentPanel.tsx` takes the same tint on all three of its branches, **full-bleed** (`-mx-5 px-5`) rather than an inset card: the window pads by `px-5`, so an indented panel would have left *CURRENT TASK* out of line with *ROTATION* and every label under it. Presentation only: no action, command or reducer change. `docs/SPEC.md` §7.2, §7.3. |
| 2026-09-07 11:05 | **The checkpoint window came back half-screen after unplugging a monitor (issue #20), and now opens on the *main* display.** Targeting the cursor's display is reversed (SPEC §8): the pointer is wherever it was abandoned, so on a two-monitor desk the checkpoint opened on whichever screen nobody was facing. `primary_monitor` — macOS's *Main display* setting — puts it in the same chosen place every time, with the cursor's monitor kept only as a fallback. It is built once and thereafter hidden and shown, but its geometry was only ever set *at build*, from the display under the cursor — so a window sized for an external monitor kept that frame after the monitor went away, landing partly off the built-in screen. `platform/checkpoint.rs` now re-fits on **every** show (`fit_to_main_monitor`: size, then position, since a resize holds the top-left corner). Units are the second half of the bug: `set_size`/`set_position` convert whatever they are handed using the **window's** scale factor — the display it is still on — so handing them the monitor's *physical* frame sized the window by the wrong factor across a Retina/1× pair, and it came out twice the screen with its content centred off the bottom-right corner. The frame is converted here with the **target monitor's** factor and passed as logical, making their conversion an identity. The window is built `visible(false)` and shown after the fit, so the one code path covers both first build and reuse and there is no flash at the default size. `docs/SPEC.md` §8. |
| 2026-09-07 10:55 | **0.4.0 built unnotarized: the credentials never reached `tauri build`.** `scripts/build-release.sh` sources `~/.secrets/timebox-release.env` and then exports what it needs — but the file exports only `APPLE_API_KEY_PATH`, and the script's `export` line named two of the four. `APPLE_API_KEY` and `APPLE_API_ISSUER` were therefore *shell variables*: visible to the preflight guard written to catch exactly this, invisible to the child process that needs them. Tauri warns on an incomplete credential set and signs without notarizing, so the run failed six minutes later at `stapler`, with `Record not found` — the same message a ticket that has not propagated gives. `spctl -a -vvv -t install` is what separates the two: *Unnotarized Developer ID* means no ticket exists and no amount of retrying will produce one. All four are exported now, and the three staples (`.app`, the copy inside the DMG, the DMG — previously one 5 × 30s loop and two bare attempts under `set -e`) share one `staple` helper at 10 × 60s whose exhaustion message leads with *not notarized*. `docs/RELEASE.md` §3. |
...
platform/       checkpoint, popover, tray windows;
                overlay: the floating timer card (issue #23) — reconciled from
                settings + state, never from an action of its own;
...
`repo::save` writes the _whole_ state in one transaction rather than diffing. At a few hundred rows a day the cost is irrelevant, and it removes the possibility of persisting half a transition. **It is called only when the reduction actually changed something** — `App::dispatch` compares the next state with the current one (issue #36). Almost every tick changes nothing, and once a second all day the whole-state write stops being irrelevant.
...
- **`docs/features/`** holds the specs that extend it, each with its own decisions and acceptance tests continuing the same numbering: `IDLE_TIME.md` (D15–D20, D22; tests 23–33, 42–47) with `IDLE_TIME_PLAN.md` beside it, `DAILY_TASKS.md` (D23–D26; tests 54–61, 107), `WEEKLY_REPORT.md` (D34–D46; tests 82–101), `TIMER_OVERLAY.md` (D47–D52; tests 102–106), and `SLEEP_DETECTION.md` (D21; tests 34–41, **not yet implemented**).
...
- Migrations store **milliseconds**, matching the core exactly; second-granularity would accumulate rounding into real drift. `001` was rewritten in place pre-release; `002` (`away_ms`, `first_run_done`), `003` (the working window and `idle_spans`), `004` (`work_spans`), `005` (`tasks.daily`), `006` (`app_state.pomodoro_since`) and `007` (the four `settings.overlay_*` columns) followed the forward-only rule. From here, add an `008`. **`006` is the first migration that rebuilds a table** rather than adding a column: `timer_state` carries a `CHECK … IN (…)` from 001, and SQLite cannot alter a CHECK in place — so any new `TimerState` variant needs the same treatment, and forgetting it fails at *runtime*, not at compile time.
...
- **A new window must be added to `src-tauri/capabilities/default.json`.** The
  `windows` list names every label the capability applies to. A window left out
  still runs the app's own `#[tauri::command]`s — they need no permission — so
  it mounts and paints its first snapshot correctly and looks fine; but
  `listen` is a *core* command and is refused, so the window never receives
  `timebox://changed` and never updates again. Nothing fails on the Rust side.
  That was issue #27, for `overlay` and silently for `settings`.
