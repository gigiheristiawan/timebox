...
| Date (WIB)       | Change                                                             |
| ---------------- | ------------------------------------------------------------------ |
| 2026-09-18 16:07 | **Declining the break at a task checkpoint resets the clock (issue #39, D53).** Answering a work checkpoint with *Complete*, *Pending* or *Extend* — the break option left unchecked — now restarts the pomodoro from zero, as *Skip & continue* does. Before, a checkpoint reached 20 min into a pomodoro re-prompted 5 min later, asking the question just answered. `decline_break` in `timer_machine.rs`; §3.2, test 110. |
...
| D53 | **Declining the break at a task checkpoint resets the clock.** *Complete*, *Pending* and *Extend* at a work checkpoint set `pomodoro_since = now` while the mode is on; with it off they leave it `None`. Added 2026-09-18 (issue #39). | The task checkpoint always offers a break, so answering it without one is the same answer as *Skip & continue* — and D29 already resets on that, for the reason that applies here word for word: a re-prompt minutes later argues with the answer just given. D31 makes the task checkpoint the richer of the two prompts; leaving it unable to reset the clock the poorer one resets made it the one that nagged. The waiting at the checkpoint is not counted either way (D28), so the reset discards only work already declined a break for. |
...
| A **task** checkpoint is answered without its break — `DecideComplete`, `DecidePending`, `DecideExtend` (D53) | `decline_break` in `reduce` |
...
| 110 | A task checkpoint answered with `DecideComplete`, `DecidePending` or `DecideExtend` resets the clock: 20 min banked at the checkpoint, and 5 min of work later no prompt fires. With the mode off, `pomodoro_since` stays `None` (D53, issue #39). |
