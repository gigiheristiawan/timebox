...
| 2026-09-10 09:35 | **§7.2/§7.3 — the current block is tinted**, in the popover and the main window alike (issue #22): accent while running, muted while paused, rest-blue on a break, and plain at either checkpoint so the waiting banner keeps its colour. The main window's panel is full-bleed rather than an inset card, so **Current task** stays aligned with every section label below it. Purely presentational — no action, state or command changes. |
| 2026-09-07 11:05 | **§8 — the checkpoint opens on the *main* display, and is re-fitted to it on every show rather than only when built (issue #20).** The cursor's display is struck as the target: the pointer sits wherever it was abandoned, so the checkpoint landed on whichever screen nobody was facing, differently each time. The main display is the one macOS's *Main display* setting names, so the checkpoint is always in the same chosen place. It is created once and thereafter hidden and shown, so it kept the frame of whatever display it was last sized for; unplug an external monitor and the checkpoint came back at that monitor's geometry, covering part of the built-in screen. `platform::checkpoint::fit_to_main_monitor` now sets size then position from `primary_monitor` (falling back to the cursor's) before each show, in **logical** points converted with the *target monitor's* scale factor — Tauri's setters convert whatever they are handed with the **window's** factor, i.e. the display it is still on, so passing physical pixels sized the window by the wrong factor across a Retina/1× pair and it came out twice the screen with its content centred off the bottom-right corner. The window is also built hidden and shown after the fit, so it no longer flashes at the default size. |
| 2026-09-04 10:55 | **Daily tasks added** — `docs/features/DAILY_TASKS.md`, D23–D26, tests 54–61 (issue #16). A task marked `daily` is never `Done` and never leaves the queue: completing one only makes it done *for today*, which is **derived** from `completed_at` against local midnight rather than stored, so no midnight reset has to fire and a quit over the day boundary cannot miss one. Rotation steps over it and `SwitchTo` refuses it until tomorrow, and completing it discards its parked block so the next day starts on a fresh allocation. `reduce` now takes `day_start` beside `now`, injected by the shell exactly as `core::summary` receives it. Migration **005**. §7.3's task editor and §7.2's popover both gain the `Daily` control and the ticked-off row. |
| 2026-08-28 15:20 | **§7.2/§7.4 — `+5 min` during a break now *adds* to the remainder.** `Event::ExtendBreak` recomputed `end_at` as `now + ms`, so extending a 10-minute break four minutes in left five minutes, not eleven — it re-granted rather than granted (issue #10). It now pushes `end_at` forward while the break runs, adds to the held remainder while it is paused (staying paused), and still resolves to `now + ms` at the break checkpoint, where nothing is left. Same rule as §7.3's "added and never assigned". Tests 53a–c. |
...
Compact, 380px wide (widened from 300 on 2026-09-14 so task titles truncate less; the width is `WIDTH` in both `platform/popover.rs` and `components/Popover.tsx`, which must agree):
- Current task title + large countdown, on a **tinted panel** that names the state at a glance (issue #22): accent while a work block runs, muted while it is paused, rest-blue during a break. At either checkpoint the panel stays plain — the checkpoint's own banner is the colour there, and a tint behind it would swallow it. The tint stops above the break and Pomodoro rows, which are controls rather than state — *Take a break* gets a warm band of its own, fading from the surface into `alert-soft` ( it never shows while a checkpoint is open, so the colour is not overloaded in practice)
- Next task preview
...
- Divider
- Today's queue (first 4–5 entries, current marked `→`), with dailies done today listed **after** every open task so they never crowd it out of the preview (issue #31; display order only — `DAILY_TASKS.md` §5.2)
- Divider
...
Sections:
- **Current task** — on the same **tinted panel** as §7.2's popover (issue #22), full-bleed so the
  section labels stay in line: accent while a work block runs, muted while paused, rest-blue on a break, plain at either
  checkpoint where `PendingDecision` above it already carries the colour. Contents: title, priority marker, countdown ring, `Pause` / `Skip` / `Complete`, extension badge if `extensionSeconds > 0`; `✎` opens an inline editor for the **title, priority and added time**
...
2. Show the expiration window on the **main display** — the one carrying the menu bar, which is what macOS's *Main display* setting names — filling it. ~~the display containing the mouse cursor~~ (struck, issue #20): the pointer is wherever it was last abandoned, so on a multi-monitor desk the checkpoint opened on a screen nobody was facing and in a different place each time. The main display is a setting, so it is the same place every time. The window is reused between checkpoints, so this is re-evaluated on every show and not only when the window is first built: a frame sized for a monitor that has since been unplugged describes no screen that still exists.
